  ...
  return (
    <form onSubmit={onSubmitHandler} className="min-h-[80vh] flex items-center">
      <div className="flex flex-col gap-3 m-auto items-start p-8 min-w-[320px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg">
        ...
        <button
          type="submit"
          className="bg-beige text-white w-full py-2 rounded-md text-base"
        >
          {state === "Sign Up" ? "Create Account" : "Login"}
        </button>
        ...
        {/* Forgot password link shown only in Login state */}
        {state === "Login" && (
          <div className="w-full text-left">
            <span
              onClick={() => navigate("/Forgot-Password")}
              className="text-beige text-sm underline cursor-pointer"
            >
              Forgot Password?
            </span>
          </div>
